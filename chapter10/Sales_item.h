...
friend bool compareIsbn(const Sales_item &,const Sales_item &);
private:
	std::string ISBN;
	int soldBok;
	double mount;
...
public:
	...
	Sales_item(std::istream &istrm){
		istrm>>this->ISBN>>this->soldBok>>this->price;
		mount=soldBok*price;}
...
bool compareIsbn(const Sales_item &,const Sales_item &);
